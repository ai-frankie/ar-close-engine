...
Recomputes the truth from raw data (AR_Aging.csv, GL.csv),
...
def bucket(due):
    ...
    d = (AS_OF - due).days
    return ("Current" if d <= 30 else "31-60" if d <= 60 else
            "61-90" if d <= 90 else "91-120" if d <= 120 else "120+")


def compute_truth(data_dir=None):
    """Recompute AR truth from raw CSVs. Returns TRUTH dict."""
    d = data_dir or D
    ...
    for a in aging:
        ob = fnum(a.get("Open_Balance"))
        b  = bucket(fdate(a.get("Due_Date")))
    ...
    writeoffs = sum(1 for a in aging
                    if bucket(fdate(a.get("Due_Date"))) == "120+"
                    and (fnum(a.get("Open_Balance")) or 0) > 5000
                    and a.get("Status") != "Disputed")
    return {
        "subledger total": subledger,
        "gl ar control":   gl_ar,
        "variance":        variance,
...
def run_review(workbook_path=None, data_dir=None):
    ...
    Returns (rows, fails) where rows = list of (metric, reported, expected, result, diagnosis).
    """
    truth = compute_truth(data_dir)
    reserve  = truth["required reserve"]
    subledger = truth["subledger total"]

    path = workbook_path or os.path.join(D, "AR_Close_Prepared_SAMPLE.xlsx")
    ...
    for key, exp in truth.items():
        got = reported(key)
        if got is None:
            rows.append((key, "(missing)", f"{exp:,.2f}", "MISSING", "Metric not found in workbook"))
            fails += 1
            continue
        ok = abs(got - exp) <= 0.5
        diag = ""
        if not ok:
            fails += 1
            if exp != 0 and abs(got / exp - 2) < 0.02:
                diag = "~2x expected - double-counted (SUM hit a total row?)"
            elif key == "true-up" and abs(got - (reserve + OPENING)) < 0.5:
                diag = "Added opening allowance instead of subtracting (sign flip)"
            elif abs(abs(got - exp) - 2 * OPENING) < 0.5:
                diag = "Off by 2× opening allowance — check +/- on the true-up"
            else:
...
def _build_sample(path, truth):
    """Build AR_Close_Prepared_SAMPLE.xlsx with two planted errors for demo."""
    reserve   = truth["required reserve"]
    subledger = truth["subledger total"]
    gl_ar     = truth["gl ar control"]
    writeoffs = truth["write-off"]
...
def write_review_report(rows, out_path=None):
    """Write AR_Close_Review.xlsx from run_review() rows."""
    ...
    path = out_path or os.path.join(D, "AR_Close_Review.xlsx")
    out.save(path)
    return path
...
if __name__ == "__main__":
    ...
    if wb_path is None:
        sample = os.path.join(D, "AR_Close_Prepared_SAMPLE.xlsx")
        if not os.path.exists(sample):
            truth = compute_truth()
            _build_sample(sample, truth)
            print("(no workbook given — built sample-with-errors:", os.path.basename(sample), ")\n")
        wb_path = sample

    rows, fails = run_review(wb_path)
    out = write_review_report(rows)
    print("AR CLOSE REVIEW —", os.path.basename(wb_path))
    print("OUTPUT:", out, "\n")
